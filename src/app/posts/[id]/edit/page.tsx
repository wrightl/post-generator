...
export default function EditPostPage() {
    ...
    useEffect(() => {
        if (appUser && Number.isInteger(postId) && postId > 0)
            load();
        else if (!authLoading) setLoading(false);
    }, [appUser, authLoading, postId, load]);
    ...
    const handleRefreshEngagement = async () => {
        if (!post?.externalPostId || refreshingEngagement) return;
        setRefreshingEngagement(true);
        setError(null);
        try {
            const updated = await refreshPostEngagement(post.id);
            setPost(updated);
        } catch (e) {
            setError(
    ...
    const handleUploadImageChange = async (
        ...
        setError(null);
        try {
            const updated = await uploadPostImage(post.id, file);
            setPost(updated);
        } catch (err) {
            setError(
    ...
    const handleRemoveImage = async () => {
        ...
        setError(null);
        try {
            const updated = await removePostImage(post.id);
            setPost(updated);
        } catch (err) {
            setError(
