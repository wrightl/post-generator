...
export default function EditPostPage() {
  ...
  useEffect(() => {
    if (appUser && idToken && Number.isInteger(postId) && postId > 0) load();
  ...
  return (
    <main className="mx-auto max-w-2xl px-6 py-8">
      ...
      <h1 className="text-2xl font-bold text-[var(--text)]">Edit post {post.id}</h1>
      <p className="mt-1 text-sm text-[var(--text-muted)]">
        Platform: {post.platform} · Created: {new Date(post.createdAt).toLocaleString()}
      </p>

      {error && (
        <p className="mt-2 rounded bg-red-100 p-2 text-red-800 dark:bg-red-900/30 dark:text-red-200" role="alert">
          {error}
        </p>
      )}
      ...
      <form onSubmit={handleSubmit} className="mt-4 space-y-4">
        <div>
          <label htmlFor="topicSummary" className={labelClass}>Topic summary</label>
          <input
            id="topicSummary"
            type="text"
            value={topicSummary}
        ...
        <div>
          <label htmlFor="script" className={labelClass}>Script (optional)</label>
          <textarea
            id="script"
            rows={3}
            value={script}
        ...
        <div>
          <label htmlFor="scheduledAt" className={labelClass}>Scheduled at (optional)</label>
          <input
            id="scheduledAt"
            type="datetime-local"
            value={scheduledAt}
